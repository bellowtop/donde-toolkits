...
torch::Tensor xyxy2xywh(const torch::Tensor& x) {
    ...
    y.index_put_({"...", 0},
                 (x.index({"...", 0}) + x.index({"...", 2})).div(2));
    y.index_put_({"...", 1},
                 (x.index({"...", 1}) + x.index({"...", 3})).div(2));
...
torch::Tensor non_max_suppression(torch::Tensor& prediction,
                                  ...
                                  float iou_thres = 0.45, int max_det = 300) {
    ...
    prediction.index_put_({"...", Slice({None, 4})},
...
torch::Tensor scale_boxes(const std::vector<int>& img1_shape,
                          torch::Tensor& boxes,
                          const std::vector<int>& img0_shape) {
    auto gain = (std::min)((float)img1_shape[0] / img0_shape[0],
                           (float)img1_shape[1] / img0_shape[1]);
    auto pad0 =
        std::round((float)(img1_shape[1] - img0_shape[1] * gain) / 2. - 0.1);
    auto pad1 =
        std::round((float)(img1_shape[0] - img0_shape[0] * gain) / 2. - 0.1);
    ...
    boxes.index_put_({"...", Slice(None, 4)},
                     boxes.index({"...", Slice(None, 4)}).div(gain));
    return boxes;
}
...
//         std::string model_path = "/Users/jiechen/Downloads/yolov8m.torchscript";
...
//             "/Users/jiechen/Downloads/test_images/test_image_2_person.jpeg");
//         cv::Mat input_image;
...
//                        scale_boxes({input_image.rows, input_image.cols}, boxes,
